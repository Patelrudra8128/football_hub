  ...
  return (
    <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden hover:border-primary/45 transition-all duration-200">
      ...
      <div className="px-4 py-2.5 bg-muted/40 border-b border-border/80 flex justify-between items-center text-[10px] text-muted-foreground font-extrabold uppercase tracking-wider">
        ...
        {isFinished && <span className="text-muted-foreground font-bold">Full Time</span>}
        {isScheduled && <span className="text-muted-foreground font-bold">Scheduled</span>}
      </div>
        ...
        <div className="flex flex-col items-center shrink-0 min-w-[90px]">
          {isScheduled ? (
            <div className="flex flex-col items-center gap-1 text-center">
              ...
              <span className="text-[9px] font-extrabold text-muted-foreground uppercase tracking-wider">{new Date(match.date).toLocaleDateString(undefined, {month: 'short', day: 'numeric'})}</span>
              <span className="text-[11px] font-bold font-mono text-foreground bg-muted/50 px-2 py-0.5 rounded border border-border/50">{new Date(match.date).toLocaleTimeString(undefined, {hour: '2-digit', minute: '2-digit'})}</span>
            </div>
          ) : (
            <div className="flex flex-col items-center gap-1">
              ...
              {isLive && <span className="text-[8px] font-black text-primary uppercase tracking-widest animate-pulse mt-1">Live</span>}
            </div>
          )}
        </div>
      ...
      <div className="px-4 py-2 border-t border-border bg-muted/10 flex justify-between items-center text-xs">
        ...
        {!isScheduled ? (
          ...
          <span className="text-[10px] text-muted-foreground font-bold">AI predictions ready</span>
        )}
              ...
              <div className="grid grid-cols-2 gap-4 text-xs font-semibold">
                <div>
                  ...
                  <ol className="list-decimal pl-4 space-y-0.5 text-muted-foreground text-[10px]">
                    ...
                    <li className="text-[9px] list-none text-primary mt-1">+6 more starters</li>
                  </ol>
                </div>
                <div>
